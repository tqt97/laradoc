...
<article
    class="rounded-lg bg-zinc-50 p-6 text-zinc-900 ring-1 ring-zinc-500/10 transition-colors ring-inset hover:border-zinc-200 dark:border-zinc-800 dark:bg-zinc-800 dark:text-zinc-100 dark:ring-zinc-700 dark:hover:border-zinc-700 group">
    <div class="space-y-3">
        <div class="text-muted-foreground text-sm">
            @if ($article->category)
                <a href="{{ route('prezet.show', ['slug' => strtolower($article->category)]) }}"
        ...
        <h2
            class="text-xl leading-tight font-semibold transition-colors duration-200 hover:opacity-75 dark:text-white group:hover:text-blue-500">
        ...
        <div class="text-muted-foreground flex items-center gap-2 text-sm dark:text-zinc-400">
            <a href="{{ route('prezet.index', ['author' => strtolower($article->frontmatter->author)]) }}"
                ...
                <div class="flex h-6 w-6 items-center justify-center overflow-hidden rounded">
                    <img src="{{ $author['image'] ?? '' }}" alt="{{ $post->author->name ?? 'Author' }}"
