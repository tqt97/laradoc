...
<div x-data="{
    ...
    <template x-teleport="body">
        <div x-cloak x-show="open" x-trap.inert.noscroll="open" x-transition:enter="transition duration-300 ease-out"
            ...
            class="fixed inset-0 z-100 overflow-x-hidden overflow-y-auto bg-zinc-900/75 p-4 backdrop-blur-xs will-change-auto md:py-8 lg:px-8 lg:py-16"
            ...
            <div x-cloak x-show="open" x-transition:enter="transition duration-300 ease-out"
                ...
                <ul x-show="filterResults.length > 0" x-ref="elListbox"
                    ...
                    <template x-for="option in filterResults" :key="option.id">
                        <li x-on:click="onOptionSelected()" x-on:mouseenter="setHighlighted(option.id, 'mouse')"
                            ...
                            class="group flex cursor-pointer flex-col rounded-2xl px-3 py-2 text-sm" role="option"
                            tabindex="-1">
                ...
                <!-- No Results Feedback -->
                <div x-show="filterResults.length === 0" class="rounded-b-3xl bg-white p-3 dark:bg-zinc-800">
