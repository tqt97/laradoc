        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-black tracking-tight text-zinc-900 dark:text-white">
                    Edit Feature: {{ $feature->key }}
                </h2>
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400 font-medium">
                    Modify feature flag status and UI properties.
                </p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-zinc-900/50 rounded-3xl border border-zinc-200 dark:border-zinc-800 p-8 shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <x-text-input id="key" name="key" type="text" class="w-full" :value="old('key', $feature->key)" placeholder="e.g. newsletter" required />
                    ...
                    <div class="space-y-2">
                        ...
                        <x-text-input id="location" name="location" type="text" class="w-full" :value="old('location', $feature->location)" placeholder="e.g. header-left" />
                    ...
                    <div class="md:col-span-2 space-y-2">
                        ...
                        <textarea id="description" name="description" rows="3" 
                            ...
                            placeholder="Briefly describe the purpose of this feature...">{{ old('description', $feature->description) }}</textarea>
                        <x-input-error :messages="$errors->get('description')" />
                    </div>

                    <div class="flex items-center gap-8 md:col-span-2">
                        <label class="relative inline-flex items-center cursor-pointer group">
                            <input type="checkbox" name="enabled" value="1" class="sr-only peer" {{ old('enabled', $feature->enabled) ? 'checked' : '' }}>
                        ...
                        <label class="relative inline-flex items-center cursor-pointer group">
                            <input type="checkbox" name="show" value="1" class="sr-only peer" {{ old('show', $feature->show) ? 'checked' : '' }}>
                            <div class="w-11 h-6 bg-zinc-200 peer-focus:outline-none rounded-full peer dark:bg-zinc-800 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-zinc-300 after:border after:rounded-full after:h-5 after:width-5 after:transition-all dark:border-zinc-600 peer-checked:bg-primary-600"></div>
            ...
            <div class="bg-white dark:bg-zinc-900/50 rounded-3xl border border-zinc-200 dark:border-zinc-800 p-8 shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <x-text-input id="ui_text" name="ui[text]" type="text" class="w-full" :value="old('ui.text', $feature->ui['text'] ?? '')" placeholder="e.g. Articles" />
                    </div>

                    <div class="space-y-2">
                        <x-input-label for="ui_icon" value="Icon Component" class="text-[10px] font-black uppercase tracking-widest text-zinc-400" />
                        <x-text-input id="ui_icon" name="ui[icon]" type="text" class="w-full" :value="old('ui.icon', $feature->ui['icon'] ?? '')" placeholder="e.g. prezet.icon-article" />
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="ui[route]" class="w-full rounded-2xl border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-zinc-900 dark:text-white focus:border-primary-500 focus:ring-primary-500 shadow-sm">
                            ...
                            @foreach ($routes as $route)
                                <option value="{{ $route }}" {{ old('ui.route', $feature->ui['route'] ?? '') == $route ? 'selected' : '' }}>{{ $route }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <x-text-input id="ui_route_active" name="ui[route_active]" type="text" class="w-full" :value="old('ui.route_active', $feature->ui['route_active'] ?? '')" placeholder="e.g. articles*" />
                    </div>
                    ...
                    <div class="md:col-span-2 space-y-2">
                        ...
                        <x-text-input id="ui_special_classes" name="ui[special_classes]" type="text" class="w-full" :value="old('ui.special_classes', $feature->ui['special_classes'] ?? '')" placeholder="e.g. text-pink-500" />
                    </div>

                    <div class="md:col-span-2 flex items-center">
                        <label class="relative inline-flex items-center cursor-pointer group">
                            <input type="checkbox" name="ui[is_special]" value="1" class="sr-only peer" {{ old('ui.is_special', $feature->ui['is_special'] ?? false) ? 'checked' : '' }}>
                            <div class="w-11 h-6 bg-zinc-200 peer-focus:outline-none rounded-full peer dark:bg-zinc-800 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-zinc-300 after:border after:rounded-full after:h-5 after:width-5 after:transition-all dark:border-zinc-600 peer-checked:bg-primary-600"></div>
                        ...
                        @foreach ($roles as $role)
                            <label class="flex items-center gap-3 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 cursor-pointer transition-all">
                                <input type="checkbox" name="roles[]" value="{{ $role->name }}" class="rounded border-zinc-300 text-primary-600 focus:ring-primary-500" 
                                    {{ is_array(old('roles', $feature->roles)) && in_array($role->name, old('roles', $feature->roles)) ? 'checked' : '' }}>
